        ...
        for country in country_rows: 
            ...
            if c[0] == ' ':
                ...
                if total_check[0].isdigit():
                    data['name'] = 'Total'
                    data['total_cases'] = int(''.join(re.findall("[0-9]", c[0]))) if c[0].strip() else 0
                    data['new_cases'] = int(''.join(re.findall("[0-9]", c[1]))) if c[1].strip() else 0
                    data['total_deaths'] = int(''.join(re.findall("[0-9]", c[2]))) if c[2].strip() else 0
                    data['new_deaths'] = int(''.join(re.findall("[0-9]", c[3]))) if c[3].strip() else 0
                    data['active_cases'] = int(''.join(re.findall("[0-9]", c[4]))) if c[4].strip() else 0
                    data['total_recovered'] = int(''.join(re.findall("[0-9]", c[5]))) if c[5].strip() else 0
                    data['condition'] = int(''.join(re.findall("[0-9]", c[6]))) if c[6].strip() else 0
                else:              
                    data['name'] = c[0].strip()
